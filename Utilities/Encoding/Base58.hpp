...
namespace Base58
{
    constexpr size_t Decodesize(size_t N) { return size_t(N * size_t{ 733 } / 1000.0f + 0.5f); }  // log(58) / log(256), rounded.
    constexpr size_t Encodesize(size_t N) { return size_t(N * size_t{ 137 } / 100.0f + 0.5f); }   // log(256) / log(58), rounded.

    namespace Internal
    {
        static constexpr auto Table = cmp::getBytes(std::array<uint8_t, 58>
        {
            '1', '2', '3', '4', '5', '6', '7', '8', '9',
            'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'J',
            'K', 'L', 'M', 'N', 'P', 'Q', 'R', 'S', 'T',
        ...
        static constexpr auto Reversetable = cmp::getBytes(std::array<uint8_t, 128>
        {
    ...
    {
        cmp::Array_t<uint8_t, Encodesize(N)> Result{}, Buffer{};
    ...
    template <cmp::Byte_t T, size_t N> requires(N == std::dynamic_extent) constexpr auto Encode(const cmp::Container_t<T, N> &Input)
    {
        cmp::Vector_t<uint8_t> Result{}, Buffer((Encodesize(Input.size())));
        size_t Outputposition{ 1 };
        size_t Leadingzeros{};
    ...
    {
        cmp::Array_t<uint8_t, Encodesize(N)> Buffer{};
        size_t Outputposition{ 1 };
        size_t Leadingzeros{};
        ...
        return (cmp::Array_t<uint8_t, Decodesize(N)>)
                cmp::resize_array<uint8_t, Encodesize(N), Decodesize(N)>(Buffer);
    }
    template <cmp::Byte_t T, size_t N> requires(N == std::dynamic_extent) constexpr auto Decode(const cmp::Container_t<T, N> &Input)
    {
        cmp::Vector_t<uint8_t> Buffer(Encodesize(Input.size()));
        size_t Outputposition{ 1 };
        size_t Leadingzeros{};
