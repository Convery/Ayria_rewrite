...
namespace FS
{
    ...
    template <cmp::Byte_t T = uint8_t> [[nodiscard]] std::basic_string<T> Readfile(std::string_view Path)
    {
        const auto Filemapping = MMap_t(std::string(Path));
        return { Filemapping.begin(), Filemapping.end() };
    }
    template <cmp::Byte_t T = uint8_t> [[nodiscard]] std::basic_string<T> Readfile(std::wstring_view Path)
    {
    ...
    // Overwrite a file.
    template <cmp::Range_t T> bool Writefile(const std::string &Path, const T &Buffer)
    {
    ...
    template <cmp::Range_t T> bool Writefile(const std::wstring &Path, const T &Buffer)
    {
        #if defined(_WIN32)
    ...
    // Append to a file.
    template <cmp::Range_t T> bool Appendfile(const std::string &Path, const T &Buffer)
    {
    ...
    template <cmp::Range_t T> bool Appendfile(const std::wstring &Path, const T &Buffer)
    {
        #if defined(_WIN32)
    ...
    [[nodiscard]] inline std::vector<std::string> Findfiles(const std::string &Directorypath, std::string_view Criteria, bool Recursive = false)
    {
    ...
    [[nodiscard]] inline std::vector<std::wstring> Findfiles(const std::wstring &Directorypath, std::wstring_view Criteria, bool Recursive = false)
    {
