...
namespace Hash
{
    namespace Internal
    {
        ...
        constexpr void Transform(std::array<uint64_t, 8> &Block, uint64_t &A, uint64_t &B, uint64_t &C)
        {
            ...
            for (size_t i = 0; i < 8; ++i) Block[i] = cmp::toLittle(Block[i]);
        ...
        constexpr std::array<uint8_t, 24> Finalize(State_t &State)
        {
            ...
            State.Block = std::bit_cast<std::array<uint64_t, 8>>(Block);
            State.Block[7] = cmp::toLittle(Messagelength);
            Transform(State.Block, State.A, State.B, State.C);

            return std::bit_cast<std::array<uint8_t, 24>>(std::array<uint64_t, 3>
            {
                cmp::toLittle(State.A),
                cmp::toLittle(State.B),
                cmp::toLittle(State.C)
            });
        }
        template <cmp::Byte_t T> constexpr void Write(State_t &State, std::span<T> Input)
        {
            ...
            if (State.Fillcount != 0 || Input.size() < sizeof(State.Block))
            {
                ...
                for (uint8_t i = 0; i < Count; ++i)
                    Block[State.Fillcount++] = (uint8_t)Input[i];
            ...
            {
                cmp::memcpy(State.Block.data(), Input.data(), 8 * sizeof(uint64_t));
    ...
    // Any range.
    template <cmp::Range_t T> constexpr cmp::Array_t<uint8_t, 24> Tiger192(const T &Input)
    {
    ...
    template <typename T> requires (!cmp::Range_t<T>) constexpr cmp::Array_t<uint8_t, 24> Tiger192(const T &Input)
    {
        return Tiger192(cmp::getBytes(Input));
    }
